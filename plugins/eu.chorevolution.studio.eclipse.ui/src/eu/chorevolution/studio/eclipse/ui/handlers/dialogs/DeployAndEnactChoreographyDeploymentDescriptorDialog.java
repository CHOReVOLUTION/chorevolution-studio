...
public class DeployAndEnactChoreographyDeploymentDescriptorDialog extends TitleAreaDialog {
	// private Image image;
	...
	private Button needToEnact;
	private Button imageToLoad;
	...
	private boolean varNeedToEnact;
	private boolean varNeedToPassImage;
	private String choreographyDescription;
	private String choreographyImageUrl;
	private Group groupFilterEnactment;
	private Group groupOtherData;
	
	private IProject project;

	public DeployAndEnactChoreographyDeploymentDescriptorDialog(Shell shell, IProject project) {
		super(shell);
	...
	protected Control createContents(Composite parent) {
		...
		setTitle("Upload and Create Choreography");

		setMessage("Upload and Create Choreography", IMessageProvider.INFORMATION);
	...
	protected Control createDialogArea(Composite parent) {
		...
		ViewLabelProvider viewLabelProvider = new ViewLabelProvider(createImageDescriptor());
		
		needToEnact = new Button(groupFilterEnactment, SWT.CHECK);
		needToEnact.setText("Enact the Choreography");
		needToEnact.addSelectionListener(new EnactSelectionListener());
		needToEnact.setSelection(false);
		
		Label labelEmpty = new Label(groupFilterEnactment, SWT.NONE);
		labelEmpty.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false, false, 2, 1));
		labelEmpty.setText("");
		...
		treeViewerAllEnactments.setLabelProvider(
				new DelegatingStyledCellLabelProvider(viewLabelProvider));
		...
		//Other data
		groupOtherData = new Group(composite, SWT.SHADOW_ETCHED_IN);
		groupOtherData.setText("Optional Fields");
		groupOtherData.setLayoutData(new GridData(SWT.FILL, SWT.BEGINNING, true, false, 1, 1));
		GridLayout layoutOtherData = new GridLayout();
		layoutOtherData.marginHeight = 3;
		layoutOtherData.marginWidth = 3;
		layoutOtherData.numColumns = 1;
		groupOtherData.setLayout(layoutOtherData);
		
		Label labelDescription = new Label(groupOtherData, SWT.NONE);
		labelDescription.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false, false, 1, 1));
		labelDescription.setText("Choreography Description: ");
		
		textDescription = new Text(groupOtherData, SWT.BORDER | SWT.MULTI);
		GridData gridDataDescription = new GridData(SWT.FILL, SWT.CENTER, true, false);
		gridDataDescription.heightHint = 5 * textDescription.getLineHeight();
		textDescription.setLayoutData(gridDataDescription);	
		
		imageToLoad= new Button(groupOtherData, SWT.BUTTON1);
		imageToLoad.setText("Load an image of the Choreography");
		imageToLoad.addMouseListener(new CustomMouseListener());
			
    	textFilterEnactments.setEnabled(false);
		filterEnactmentsButton.setEnabled(false);
		treeViewerAllEnactments.getControl().setEnabled(false);	  
	...
	private void validateEnableOrDisableOKButton() {
		...
		if(needToEnact.getSelection()) {//i need to select an enactment engine
			if (!selection.isEmpty()) {
				Object selectedElement = selection.getFirstElement();
				if (selectedElement instanceof EnactmentEngine) {
					getButton(IDialogConstants.OK_ID).setEnabled(true);
					getButton(IDialogConstants.OK_ID).setFocus();
					setMessage("Deploy/Enact Choreography Deployment Descriptor.");
				}
			} else {
	...
	private class ViewLabelProvider extends LabelProvider implements IStyledLabelProvider {
		...
		public void dispose() {
			...
			if(choreographyImageUrl==null) 
				varNeedToPassImage = false;
			else {
				varNeedToPassImage = true;
			}
			
			choreographyDescription = textDescription.getText();
	...
	private class EnactSelectionListener implements SelectionListener {

		@Override
		public void widgetSelected(SelectionEvent e) {
			Button button = (Button) e.widget;

			varNeedToEnact=needToEnact.getSelection();
			
			if (button.getSelection()) {
				
				textFilterEnactments.setEnabled(true);
				filterEnactmentsButton.setEnabled(true);
				treeViewerAllEnactments.getControl().setEnabled(true);
	...
	private class CustomMouseListener implements MouseListener {
		...
		public void mouseDown(MouseEvent e) {
			imageDialog = new FileDialog(getShell(), SWT.OPEN);
			imageDialog.setFilterExtensions(new String [] {"*.jpg"});
			imageDialog.setFilterPath("");
			choreographyImageUrl = imageDialog.open();
			
			if(choreographyImageUrl == null)
				imageToLoad.setText("Load an image of the Choreography");
			else
				imageToLoad.setText("Image loaded");
		}
	...
	public String getDescription() {
		return choreographyDescription;
	}
	
	public boolean isNeedToEnact() {
		return varNeedToEnact;
	}
	
	public boolean isNeedToPassImage() {
		return varNeedToPassImage;
	}
	
	public String getChoreographyImageUrl() {
		return choreographyImageUrl;
	}
